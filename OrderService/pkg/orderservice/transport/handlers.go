...
func CreateServer(repository model.OrderRepository) *Server {
	return &Server{
...
func (srv Server) getOrders(w http.ResponseWriter, _ *http.Request) {
	orders, err := srv.orderRepository.GetAll()
	if err != nil {
...
func (srv Server) getOrder(w http.ResponseWriter, r *http.Request) {
	...
	order, err := srv.orderRepository.GetById(orderId)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
...
func (srv Server) createOrder(w http.ResponseWriter, r *http.Request) {
	...
	orderId, err := srv.orderRepository.Add(order)
	if err != nil {
		log.Fatal(err)
